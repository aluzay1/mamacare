...
def admin_login():
    try:
        ...
        # Find admin user (admin or super_admin)
        admin = User.query.filter_by(email=email).filter(User.role.in_(['admin', 'super_admin'])).first()
...
def admin_detail(admin_id):
    try:
        admin = User.query.filter_by(id=admin_id).filter(User.role.in_(['admin', 'super_admin'])).first_or_404()
        ...
        if request.method == 'GET':
            ...
            if admin.role == 'super_admin' and super_admin_count <= 1:
                return jsonify({'error': 'Cannot delete the last super administrator'}), 400
